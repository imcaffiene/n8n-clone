...
export function BaseNode({ className, status, ...props }: BaseNodeProps) {
  return (
    ...
    >
      {props.children}
      {status === "error" && (
        <CircleXIcon animateOnHover className="absolute right-0.5 bottom-0.5 size-2 text-red-700 stroke-3" />
      )}
      {status === "success" && (
        <CircleCheckIcon animateOnHover className="absolute right-0.5 bottom-0.5 size-2 text-green-700 stroke-3" />
      )}
      {status === "loading" && (
        <LoaderCircleIcon animateOnView className="absolute right-0.5 bottom-0.5 size-2 text-blue-700 stroke-3" />
      )}
    </div>
  );
}
